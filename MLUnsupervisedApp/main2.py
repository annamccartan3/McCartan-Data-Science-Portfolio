...
def get_model(name, params):
    if name == "Principal Component Analysis":
        ...
        model = AgglomerativeClustering(n_clusters=params["n_clusters"], linkage="ward", random_state=st.session_state['random_state'])
    return model
...
if st.session_state['welcome_screen']:
    ...
    if data_source == "Upload CSV":
        ...
        if uploaded_file:
            ...
            # Drop & Encode incompatible columns
            ...
            # Detect columns with >50% missing values
            missing_thresh = 0.5
            high_missing_cols = df.columns[df.isnull().mean() > missing_thresh].tolist()
            # Combine recommendations (avoiding duplicates)
            recommended_drop = list(set(non_numeric_cols + high_missing_cols))
            # Multiselect for columns to drop
            drop_cols = st.sidebar.multiselect(
                "Columns recommended to drop (non-numeric or high missing %):",
                options=df.columns.tolist(),
                default=recommended_drop
            )
            # Show detected columns separately (info only)
            if non_numeric_cols:
            ...
            if non_numeric_cols:
                encode_cols = st.sidebar.multiselect(
                    ...
                    options=[col for col in non_numeric_cols if col not in drop_cols],
                    default=[]
                )
            # Apply drops
            ...
            # Store processed DataFrame and info
            st.session_state['df'] = df_encoded
            st.session_state['drop_cols'] = drop_cols
            st.session_state['encoded_cols'] = encode_cols
            ...
            st.session_state['missing_option'] = missing_option # Store in session_state
    ...
    st.sidebar.success(f"Parameter saved! Random state: {random_state}")

    # Variable selection
    if 'df' in st.session_state:
        df = st.session_state['df']
        st.sidebar.subheader("Choose Variables")

        # Select target variable
        with st.sidebar.expander("Select target variable"):
            target_col = st.selectbox("Choose target column", df.columns, index=len(df.columns)-1)

        # Define default feature list (exclude drop_cols and target)
        drop_cols = st.session_state.get('drop_cols', [])
        if drop_cols is None:
            drop_cols = []
        default_features = [col for col in df.columns if col not in drop_cols and col != target_col]

        # Select features
        with st.sidebar.expander("Select feature variables"):
            selected_features = st.multiselect("Select feature columns", default_features, default=default_features)

        # Store selections in session_state
        st.session_state['selected_features'] = selected_features
        st.session_state['X'] = df[selected_features]
        st.session_state['y'] = df[target_col]  # ← Fix: assign actual column values, not the column name
            ...
            if "model_name" in st.session_state:
                ...
                if st.session_state.get("model_name") == "K-Means Clustering":
                    ...
                    if "X_scaled" in st.session_state:
                        ...
                        # Dropdown to select coloring mode
                        color_mode = st.selectbox("Color points by", ["K-Means Clusters", "Target Labels"])

                        # Fit KMeans (only for clustering color mode)
                        if color_mode == "K-Means Clusters":
                            ...
                            clusters = None  # We'll use st.session_state["y"]

                        # PCA Projection
                        pca = PCA(n_components=2)
                        st.session_state["X_pca"] = X_pca

                        # Determine labels and coloring values
                        if color_mode == "Target Labels":
                            ...
                            label_names = (
                                ...
                                if len(np.unique(y)) <= 10 else [f"Class {i}" for i in labels]
                            )
                        else:
                            labels = np.unique(clusters)
                            color_labels = clusters
                            label_names = [f"Cluster {i}" for i in labels]

                        # Optional centroids
                        show_centroids = color_mode == "K-Means Clusters" and st.checkbox("Show Cluster Centroids", value=True)
                        ...
                        if show_centroids:
                            centroids_2d = pca.transform(kmeans.cluster_centers_) * scale_factor
                            ax.scatter(
                        ...
                        ax.set_title(f"2D PCA Projection ({color_mode})")
                        ax.legend(loc="best")
                        ax.grid(True)
